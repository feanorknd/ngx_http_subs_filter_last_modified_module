...
static ngx_command_t  ngx_http_subs_filter_commands[] = {
    ...
    { ngx_string("subs_filter_last_modified"),
      ...
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_subs_loc_conf_t, last_modified),
      NULL },
...
static ngx_http_module_t  ngx_http_subs_filter_last_modified_module_ctx = {
...
ngx_module_t  ngx_http_subs_filter_last_modified_module = {
    NGX_MODULE_V1,
    &ngx_http_subs_filter_last_modified_module_ctx,      /* module context */
...
ngx_http_subs_header_filter(ngx_http_request_t *r)
{
    ...
    slcf = ngx_http_get_module_loc_conf(r, ngx_http_subs_filter_last_modified_module);
...
ngx_http_subs_init_context(ngx_http_request_t *r)
{
    ...
    ngx_http_set_ctx(r, ctx, ngx_http_subs_filter_last_modified_module);
...
ngx_http_subs_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    ctx = ngx_http_get_module_ctx(r, ngx_http_subs_filter_last_modified_module);
    if (ctx == NULL) {
        return ngx_http_next_body_filter(r, in);
    }
    ...
    ngx_http_subs_ctx_t *ctx)
{
    ...
    if (ctx->free) {
        ...
        temp->buf->tag = (ngx_buf_tag_t) &ngx_http_subs_filter_last_modified_module;
        temp->buf->recycled = 1;
                     ...
                     ngx_chain_t *in)
{
    ...
    ngx_chain_update_chains(r->pool, &ctx->free, &ctx->busy, &ctx->out,
                            (ngx_buf_tag_t) &ngx_http_subs_filter_last_modified_module);
#else
